...
public class LeftCrossBow : MonoBehaviour
{
    public InputActionReference leftShoot; // 컨트롤러(트리거)키 눌림 체크
    public XRBaseController haptic; // 사격 시 컨트롤러 진동
    public GameObject leftArrow;              // 화살 프리팹
    ...
    private void OnEnable()
    {
        if (leftShoot != null)
        {
            leftShoot.action.Enable();
            leftShoot.action.performed += OnTriggerPressed;
        }
        isFire = false;
    }
    ...
    private void Fire()
    {
        ...
        // 화살 생성
        ArrowInstance = Instantiate(leftArrow, firePos.position, firePos.rotation);
